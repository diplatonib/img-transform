...
// ../projects/trueconf/php/online-service/site/project/
// 

const settings = {
    fileType: /\.twig/,
    projectFolder: '../projects/trueconf/php/online-service/site/project/',
    // projectFolder: './test/',
    folderName: '../projects/trueconf/php/online-service/site/project/content/ru/',
    // folderName: './test/twig/',
    encode: 'utf-8',
...
function createDOM(err, fileContent) {
    ...
    function writeImagesPathsResultFile(paths) {
        paths = paths.join('\n');
        // fs.writeFileSync(settings.imagesList, '');
        fs.appendFile(settings.imagesList, (paths) + '\n', 'utf8', (err) => {
            if (err) throw err;
        });
    ...
    let imagesPathsResolve = srcArray => {
        ...
        let resolvePaths = function(src) {
            let resolved = path.resolve(src.replace(/^\//, settings.projectFolder));
    ...
    console.log(chalk.underline('Input to compresss:'), (imagesPaths));
...
function compressAndResizeImages() {

    let imagesPathsToCompressArray = fs.readFileSync(settings.imagesList, settings.encode, err => {
        if (err) { throw err };
    });

    console.log(`\nCompress module:\n${imagesPathsToCompressArray}`);
}
